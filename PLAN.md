...
- **Plus — the cassette fine print.** The "runs with no API key" claim has two limits. State both rather than letting a reviewer discover them:
  1. **It holds for *unchanged* code.** Clone the repo, change `k` to 1 to watch the gate fail, and you get a cassette-miss error instead of a demo. Either say so in one README line, or commit a second cassette set recorded from the deliberately-broken config so the failure path is reproducible keyless too.
  2. **Replayed injection ASR guards a narrower surface than it sounds like.** A prompt change invalidates every cassette key and forces a re-record, so tier-1 ASR cannot catch a regression *introduced by a prompt edit* — it guards retrieval, parsing, and corpus changes. Still worth gating on every push. `ARCHITECTURE.md` should name which regressions tier 1 actually catches instead of implying all of them.
  - Understating this costs nothing. Overstating it is the one claim in the repo a sharp reviewer can disprove in about two minutes at the keyboard.
- **Gate:** walk the research doc's **MVP acceptance checklist** end to end, every box ticked · both cassette limits documented — README for (1), `ARCHITECTURE.md` for (2) · hand the README to someone (or a fresh Claude session) with 3 minutes and ask what the project does — if they say "a RAG chatbot," the positioning failed and you rewrite the opening.
- **Checkpoint:** A CTO and a QA lead read this repo. What does each need to see in the first 30 seconds? And: which sentence in your README is the easiest one for a reviewer to disprove at the keyboard?
